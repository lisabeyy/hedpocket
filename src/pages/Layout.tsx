...
import { useHashconnectService } from '../lib/hashconnect';
...
//hashconnect
  const { connectToExtension , disconnect , requestAccountInfo, pairingData , availableExtension , network , pairingString } = useHashconnectService();

 
 
  const handleClick = () => {
    console.log('availableExtension' , availableExtension);
    if (availableExtension && !pairingData)  connectToExtension().then(r => {
      console.log('connect to extension  resutl', r)
    });
    else if(pairingData) {
      disconnect();
      setUserAddress('');
  ...
  useEffect(() => {
    ...
    if(pairingData && pairingData.accountIds) {
      setUserAddress(pairingData.accountIds[0]);
      if (address == '') {
  ...
  const handleSetSearchAddress= (address: string) => {
      ...
      <div className='h-full'>
        ...
        <div className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
          ...
          <div className="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-900 px-6 pb-4">
            <div className="flex h-16 shrink-0 items-center">
              { /* logo venom */}
              <img src={Logo} width={100} alt="logo " /><span className='text-white text-xs font-bold pl-1 '>Mainnet</span>
        ...
        <div className="lg:pl-72 h-full">
          ...
          {/*   {!userAddress && (
             ...
             <button  className="btn text-black hover:text-[#05ED9F]" onClick={handleClick}>{pairingData ? "Disconnect" : "Connect"}</button>
          ...
          <Routes>
            ...
            <Route path="/wallet/:address?" element={<Wallet userAddress={userAddress || pairingData?.accountIds[0]} address={address} onSetSearchAddress={handleSetSearchAddress}/>} />
            <Route path="/watchlist" element={<Watchlist addressRedirect={address} userAddress={userAddress || pairingData?.accountIds[0]} />} />
          </Routes>
